...
public class Barra implements Serializable {
    ...
    public void createBarModel() {
        ...
        barDataSet.setLabel("Reportes Gráficos");
		...
		// LISTA DE LOS VALORES CUANTITATIVOS QUE IRÁN EN EL GRÁFICO
		List<Number> values = new ArrayList<>();
		// LISTA DE LOS VALORES CUALITATIVOS QUE IRÁN EN EL GRAFICO
		List<String> labels = new ArrayList<>();
		...
		try {
			if(fecha!=null) {
				...
				barDataSet.setLabel("Número de incidentes por tipo en  "+ (fecha.getYear()+1900)+"/"+mes+"/"+dia);
				// Datos Cuantitativos
					...
					} else 
					{
						...
						} else 
						{
							if(nombreDivision!=null) {
								barDataSet.setLabel("Reportes por oficial de la división "+nombreDivision+" ");
								q = em.createNativeQuery("SELECT * from dim_incidente_llamadas, \r\n"
